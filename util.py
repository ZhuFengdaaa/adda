...
def load_variables_redir(load_path, source_pre, target_pre, variables=None, sess=None):
    sess = sess or get_session()
    variables = variables or tf.trainable_variables()
    ...
    if isinstance(loaded_params, list):
        ...
        for v in variables:
            if v.name.startswith(source_pre):
                for k in loaded_params.keys():
                    if k.startswith(target_pre):
                        if v.name[len(source_pre):] == k[len(target_pre):]:
                            restores.append(v.assign(loaded_params[k]))
    
    print("load_variables_redir", restores)
    sess.run(restores)
# ================================================================
